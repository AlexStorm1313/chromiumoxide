...
use chromiumoxide_cdp::cdp::browser_protocol::page::{
	self, AddScriptToEvaluateOnNewDocumentParams, CreateIsolatedWorldParams, EventFrameDetached,
	EventFrameStartedLoading, EventFrameStoppedLoading, EventLifecycleEvent,
	EventNavigatedWithinDocument, Frame as CdpFrame, FrameId, FrameTree,
};
...
impl FrameManager {
	...
	pub fn on_frame_execution_context_destroyed(&mut self, event: &EventExecutionContextDestroyed) {
		if let Some(id) = self
			.context_ids
			.remove(&event.execution_context_unique_id.clone().into())
		{
			if let Some(frame) = self.frames.get_mut(&id) {
				frame.destroy_context(event.execution_context_unique_id.clone().into());
			}
		}
	}
